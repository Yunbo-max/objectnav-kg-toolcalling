...
# Original used OpenAI API. We use local Qwen2.5-7B via local_vlm module.
LOCAL_MODEL_PATHS = [
                ...
                if len(target_point_map) > 0:
                    ...
                    while retries > 0:  
                        try: 
                            ...
                            last_decision.clear()
                            for i in range(num_agents):
                                fi = goal_frontiers.get("robot_"+ str(i), 0)
...
# Override chat_completion_create to use MiniMax API
import openai as _oai
_brain = _oai.OpenAI(
    api_key=__import__('os').environ.get("SILICONFLOW_API_KEY", ""),
    base_url="https://api.siliconflow.cn/v1"
)
_original_chat = chat_completion_create
def chat_completion_create(model, messages, temperature=0, **kwargs):
    try:
        r = _brain.chat.completions.create(
            model=__import__('os').environ.get("BRAIN_MODEL", "Pro/MiniMaxAI/MiniMax-M2.5"),
            messages=messages, max_tokens=100, temperature=temperature
        )
        return {"choices": [{"message": {"content": r.choices[0].message.content}}]}
    except Exception as e:
        print(f"API error: {e}")
        return _original_chat(model, messages, temperature=temperature)
