...
class LLM_Agent_GT(Agent):
    ...
    def _plan(self, planner_inputs):
        ...
        if stop and planner_inputs['found_goal'] == 1:
            ...
            use_look_actions = not getattr(self.args, "disable_look_actions", False)
            if use_look_actions and exp_pred[eve_start_x, eve_start_y] == 0 and self.eve_angle > -60:
                action = 5
                self.eve_angle -= 30
            elif use_look_actions and exp_pred[eve_start_x, eve_start_y] == 1 and self.eve_angle < 0:
                action = 4
                self.eve_angle += 30
