...
def write_episode_jsonl(args, episode_idx, success, spl, extra=None):
...
def _float_scores(values, expected, default):
    try:
        ...
        return list(default)
    if len(scores) != expected or not all(np.isfinite(scores)):
        return list(default)
    total = sum(scores)
    if total <= 0:
        return list(default)
...
def _binary_scores(weighted, raw=None):
    if isinstance(weighted, tuple) and len(weighted) == 2:
        return tuple(_float_scores(weighted, 2, [0.5, 0.5]))
    return tuple(_float_scores(raw, 2, [0.5, 0.5]))


def _choice_scores(weighted, raw=None):
    if isinstance(weighted, tuple) and len(weighted) == 4:
        return tuple(_float_scores(weighted, 4, [0.25, 0.25, 0.25, 0.25]))
    return tuple(_float_scores(raw, 4, [0.25, 0.25, 0.25, 0.25]))
...
def main():
    ...
    device = torch.device(f"cuda:{args.sem_gpu_id}" if args.cuda else "cpu")
...
###########################################################===Load Models and Datasets===########################################################
    ...
    for agent_name in config_env.SIMULATOR.AGENTS[:config_env.SIMULATOR.NUM_AGENTS]:
        config_env.SIMULATOR[agent_name].SENSORS = agent_sensors
    ...
    habitat_agg_metrics: Dict = defaultdict(float)
    ...
    count_episodes = max(0, int(args.start_episode))
    if count_episodes > 0:
        skip_episodes = min(count_episodes, num_episodes)
        for _ in range(skip_episodes):
            env.reset()
        logging.info(
            "Resuming evaluation after skipping %d completed episodes",
            skip_episodes,
        )
    count_step = 0
    goal_points = []
            ...
            if agent[0].l_step % args.num_local_steps == args.num_local_steps - 1 or agent[0].l_step == 0:
                ...
                if len(full_target_point_map) > 0:
                    ...
                    for j in range(num_agents):
                        ...
                        Perception_PR = _binary_scores(Perception_PR, Perception_Rel)
                        ...
                        if len(agent_TargetPointMap[j]) > 0:
                            ...
                            if len(history_nodes) > 0:
                                ...
                                FN_PR = _binary_scores(FN_PR, FN_Rel)
                            ...
                            if FN_PR[0] >= 0.5 or agent[j].l_step <= 125:
                                ...
                                Final_PR = _choice_scores(Final_PR, Meta_Score)
                                Meta_Score = _choice_scores(Meta_Score)
                            ...
                            if FN_PR[0] >= 0.5 or agent[j].l_step <= 125:
                                ...
                                Choice2 = list(Meta_Score).index(max(Meta_Score))
...
##############################################===Metrics===##############################################
        ...
        habitat_success = float(habitat_metrics.get("success", 0.0))
        habitat_spl = float(habitat_metrics.get("spl", habitat_metrics.get("SPL", 0.0)))
        habitat_agg_metrics["success"] += habitat_success
        habitat_agg_metrics["spl"] += habitat_spl
        log += "\nhabitat_success: {:.3f}, habitat_spl: {:.3f}, ".format(
            habitat_agg_metrics["success"] / count_episodes,
            habitat_agg_metrics["spl"] / count_episodes,
        )
        log += "episode_habitat_success: {:.0f}, episode_habitat_spl: {:.3f}".format(
            habitat_success,
            habitat_spl,
        )
        ...
        write_episode_jsonl(
            ...
            habitat_spl,
            {
                "metric_source": "habitat",
                "habitat_success": habitat_success,
                "habitat_spl": habitat_spl,
                "distance_to_goal": float(habitat_metrics.get("distance_to_goal", np.nan)),
            },
        )
        logging.info(log)
