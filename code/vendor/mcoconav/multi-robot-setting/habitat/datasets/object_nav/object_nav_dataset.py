...
def _normalize_quat_xyzw(rotation: Any) -> Any:
    if rotation is None:
        return rotation
    try:
        vals = [float(v) for v in rotation]
...
class ObjectNavDatasetV1(PointNavDatasetV1):
    ...
    def __deserialize_goal(serialized_goal: Dict[str, Any]) -> ObjectGoal:
        ...
        for vidx, view in enumerate(g.view_points):
            ...
            view_location.agent_state.rotation = _normalize_quat_xyzw(
                view_location.agent_state.rotation
            )
    ...
    def from_json(
        ...
        for i, episode in enumerate(deserialized["episodes"]):
            ...
            episode.start_rotation = _normalize_quat_xyzw(
