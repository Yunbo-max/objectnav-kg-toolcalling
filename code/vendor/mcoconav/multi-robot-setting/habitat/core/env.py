...
class Env:
    ...
    def reset(self) -> Observations:
        ...
        for obj in scene.objects:
            self.cur_mapping[obj.category.name()] = obj.category.index()

        instance_id_to_label_id = {int(obj.id.split("_")[-1]): obj.category.index() for obj in scene.objects}
        self.mapping = np.array([ instance_id_to_label_id[i] for i in range(len(instance_id_to_label_id)) ])
        for i in range(len(observations)):
            if 'semantic' in observations[i]:
                if len(self.mapping) > 0:
                    for se1 in range(len(observations[i]['semantic'])):
                        for se2 in range(len(observations[i]['semantic'][se1])):
                            if observations[i]['semantic'][se1][se2] >= len(self.mapping):
                                observations[i]['semantic'][se1][se2] = len(self.mapping)-1
                    observations[i]['semantic'] = np.take(self.mapping, observations[i]['semantic'])
                observations[i]['cur_mapping'] = self.cur_mapping
            # print(observations[i]['semantic'])
